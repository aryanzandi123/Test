...
def start_query():
    ...
    # Extract interactor discovery round skip options (default: False)
    skip_round_1a = bool(data.get('skip_round_1a', False))
    skip_round_1b = bool(data.get('skip_round_1b', False))
    skip_round_1c = bool(data.get('skip_round_1c', False))
    skip_round_1d = bool(data.get('skip_round_1d', False))
    skip_round_1e = bool(data.get('skip_round_1e', False))
    skip_round_1f = bool(data.get('skip_round_1f', False))
    skip_round_1g = bool(data.get('skip_round_1g', False))

    # Extract function discovery round skip options (default: False)
    skip_round_2a = bool(data.get('skip_round_2a', False))
    skip_round_2a2 = bool(data.get('skip_round_2a2', False))
    skip_round_2a3 = bool(data.get('skip_round_2a3', False))
    skip_round_2a4 = bool(data.get('skip_round_2a4', False))
    skip_round_2a5 = bool(data.get('skip_round_2a5', False))
    skip_round_2b = bool(data.get('skip_round_2b', False))
    skip_round_2b2 = bool(data.get('skip_round_2b2', False))
    skip_round_2b3 = bool(data.get('skip_round_2b3', False))

    # Extract post-processing skip options (default: False)
    ...
    skip_pmid_validation = bool(data.get('skip_pmid_validation', False))
    skip_metadata_generation = bool(data.get('skip_metadata_generation', False))
    skip_schema_validation = bool(data.get('skip_schema_validation', False))
    skip_function_cleaning = bool(data.get('skip_function_cleaning', False))
    ...
    with jobs_lock:
        ...
        thread = threading.Thread(
            target=run_full_job,
            args=(
                protein_name, jobs, jobs_lock, interactor_rounds, function_rounds, max_depth,
                # Interactor discovery round skips
                skip_round_1a, skip_round_1b, skip_round_1c, skip_round_1d,
                skip_round_1e, skip_round_1f, skip_round_1g,
                # Function discovery round skips
                skip_round_2a, skip_round_2a2, skip_round_2a3, skip_round_2a4,
                skip_round_2a5, skip_round_2b, skip_round_2b2, skip_round_2b3,
                # Post-processing skips
                skip_validation, skip_deduplicator, skip_arrow_determination, skip_fact_checking,
                skip_pmid_validation, skip_metadata_generation, skip_schema_validation, skip_function_cleaning,
                app
            )
        )
        thread.daemon = True
...
def build_full_json_from_db(protein_symbol: str) -> dict:
    ...
    if len(interactor_proteins) > 1:
        ...
        # EXCLUDE interactions discovered in THIS query (only show cross-links from OTHER queries)
        shared_interactions = db.session.query(Interaction).filter(
            ...
            Interaction.discovered_in_query != protein_symbol  # Exclude this query's discoveries
        ).all()
...
Arrows: -> (activates), -| (inhibits), <-> (binds)"""
    ...
    if not interactions:
        ...
        for i, inter in enumerate(interactions, 1):
            ...
            arrow = inter.get("arrow", "unk")
            direction = inter.get("direction", "unk")
