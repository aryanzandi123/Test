...
def deep_merge_interactors(
    ...
    for new_int in new_interactors:
        ...
        if primary_key in interactor_map:
            ...
            # Merge functions (append new ones)
            existing_functions = existing.get("functions", [])
            new_functions = new_int.get("functions", [])

            # TAG new functions with context metadata before appending
            if new_functions:
                # Determine context type based on interaction classification
                ...
                if interaction_type == "indirect" or upstream or mediator_chain:
                    context_type = "chain"
                else:
                    context_type = "direct"

                # Tag each new function with context
                tagged_functions = []
                for fn in new_functions:
                    if isinstance(fn, dict):
                        # Add context metadata if not already present
                        if "_context" not in fn:
                            fn["_context"] = {
                                "type": context_type,
                                "query_protein": new_int.get("_query_protein"),  # Can be added during pipeline
                                "chain": mediator_chain if mediator_chain else None
                            }
                        tagged_functions.append(fn)
                    else:
                        tagged_functions.append(fn)

                existing["functions"] = existing_functions + tagged_functions
...
def aggregate_function_arrows(interactor: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Determine backward-compat arrow field
    all_arrows = set()
    ...
    if len(all_arrows) == 0:
        ...
        arrow = "complex"  # Mixed arrow types
...
def _run_main_pipeline_for_web(
    ...
    arrow_trigger_ready = False  # Unlock arrow determination once Step 2b3 (or later) runs
...
def run_full_job(
    ...
    skip_round_1a: bool = False,
    skip_round_1b: bool = False,
    skip_round_1c: bool = False,
    skip_round_1d: bool = False,
    skip_round_1e: bool = False,
    skip_round_1f: bool = False,
    skip_round_1g: bool = False,
    # Function discovery round skips
    skip_round_2a: bool = False,
    skip_round_2a2: bool = False,
    skip_round_2a3: bool = False,
    skip_round_2a4: bool = False,
    skip_round_2a5: bool = False,
    skip_round_2b: bool = False,
    skip_round_2b2: bool = False,
    skip_round_2b3: bool = False,
    # Post-processing skips
    skip_validation: bool = False,
    skip_deduplicator: bool = False,
    skip_arrow_determination: bool = False,
    skip_fact_checking: bool = False,
    skip_pmid_validation: bool = False,
    skip_metadata_generation: bool = False,
    skip_schema_validation: bool = False,
    skip_function_cleaning: bool = False,
    flask_app = None
):
    """
    ...
    Args:
        ...
        Interactor discovery round skips (Phase 1):
        skip_round_1a: Skip Round 1a - Initial discovery (~90s, 15-25 proteins)
        skip_round_1b: Skip Round 1b - Expand network (~75s, 10-15 proteins)
        skip_round_1c: Skip Round 1c - Deep mining (~120s, 8-12 proteins)
        skip_round_1d: Skip Round 1d - Round 2 discovery (~60s, 5-10 proteins)
        skip_round_1e: Skip Round 1e - Round 3 discovery (~50s, 5-8 proteins)
        skip_round_1f: Skip Round 1f - Round 4 discovery (~40s, 3-6 proteins)
        skip_round_1g: Skip Round 1g - Round 5 final sweep (~30s, 3-5 proteins)

        Function discovery round skips (Phase 2):
        skip_round_2a: Skip Round 2a - First batch (~120s, 8-10 proteins)
        skip_round_2a2: Skip Round 2a2 - Second batch (~120s, 8-10 proteins)
        skip_round_2a3: Skip Round 2a3 - Exhaustive sweep (~180s, all remaining)
        skip_round_2a4: Skip Round 2a4 - Round 2 additional (~90s)
        skip_round_2a5: Skip Round 2a5 - Round 3 final (~60s)
        skip_round_2b: Skip Round 2b - Deep research (~120s, rare functions)
        skip_round_2b2: Skip Round 2b2 - Indirect functions (~90s, chain context)
        skip_round_2b3: Skip Round 2b3 - Rescue direct (~60s, ensure coverage)

        Post-processing skips (Phase 3):
        skip_validation: Skip evidence validation (~120s, less thorough quotes)
        skip_deduplicator: Skip function deduplication (~150s, may have duplicates)
        skip_arrow_determination: Skip LLM arrow determination (~300s, uses heuristic, 100× faster)
        skip_fact_checking: Skip claim fact-checking (~180s, unverified claims)
        skip_pmid_validation: Skip PMID validation (~30s, may have invalid citations)
        skip_metadata_generation: Skip metadata generation (~5s, missing interaction patterns)
        skip_schema_validation: Skip schema validation (~15s, inconsistent arrows/chains)
        skip_function_cleaning: Skip function cleaning (~5s, verbose function names)
    ...
    # Filter out skipped rounds based on user configuration
    skip_step_names = []

    # Map skip flags to step names
    if skip_round_1a: skip_step_names.append("step1a_discover")
    if skip_round_1b: skip_step_names.append("step1b_expand")
    if skip_round_1c: skip_step_names.append("step1c_deep_mining")
    if skip_round_1d: skip_step_names.append("step1d_discover_round2")
    if skip_round_1e: skip_step_names.append("step1e_discover_round3")
    if skip_round_1f: skip_step_names.append("step1f_discover_round4")
    if skip_round_1g: skip_step_names.append("step1g_discover_round5")

    if skip_round_2a: skip_step_names.append("step2a_functions")
    if skip_round_2a2: skip_step_names.append("step2a2_functions_batch")
    if skip_round_2a3: skip_step_names.append("step2a3_functions_exhaustive")
    if skip_round_2a4: skip_step_names.append("step2a4_functions_round2")
    if skip_round_2a5: skip_step_names.append("step2a5_functions_round3")
    if skip_round_2b: skip_step_names.append("step2b_deep_function_research")
    if skip_round_2b2: skip_step_names.append("step2b2_indirect_functions")
    if skip_round_2b3: skip_step_names.append("step2b3_rescue_direct_functions")

    # Filter pipeline steps
    if skip_step_names:
        original_count = len(pipeline_steps)
        pipeline_steps = [step for step in pipeline_steps if step.name not in skip_step_names]
        filtered_count = original_count - len(pipeline_steps)
        if filtered_count > 0:
            print(f"[SKIP] Filtered out {filtered_count} skipped rounds: {skip_step_names}", file=sys.stderr)
    ...
    if not skip_metadata_generation and METADATA_GENERATOR_AVAILABLE and generate_interaction_metadata is not None:
        post_steps += 1

    # Schema validation - pre-gate (conditional)
    if not skip_schema_validation and SCHEMA_VALIDATOR_AVAILABLE and validate_schema_consistency is not None:
        post_steps += 1
    ...
    if not skip_schema_validation and SCHEMA_VALIDATOR_AVAILABLE and finalize_interaction_metadata is not None:
        post_steps += 1

    # PMID updates (conditional)
    if not skip_pmid_validation and PMID_UPDATER_AVAILABLE and update_payload_pmids is not None:
        post_steps += 1

    # Function name cleaning (conditional)
    if not skip_function_cleaning and FUNCTION_NAME_CLEANER_AVAILABLE and clean_payload_function_names is not None:
        post_steps += 1
    ...
    try:
        ...
        # --- STAGE 2: Run evidence validation FIRST (right after pipeline) ---
        if skip_validation:
            # Skip validation as requested by user (no progress update needed)
            validated_payload = pipeline_payload
        elif not VALIDATOR_AVAILABLE:
            # Validator not available (no progress update needed)
            validated_payload = pipeline_payload
        else:
            current_step += 1
            update_status(
            ...
            validated_payload = validate_and_enrich_evidence(
                pipeline_payload,
                api_key,
                verbose=False,  # Verbose is always off for the web version
                step_logger=step_logger
            )

        # --- STAGE 2.5: Generate interaction-level metadata from function-level data ---
        if not skip_metadata_generation and METADATA_GENERATOR_AVAILABLE and generate_interaction_metadata is not None:
            current_step += 1
            update_status(
        ...
        # --- STAGE 2.6: Pre-Validation Gate (Schema Consistency) ---
        if not skip_schema_validation and SCHEMA_VALIDATOR_AVAILABLE and validate_schema_consistency is not None:
            current_step += 1
            update_status(
            ...
            validated_payload = validate_schema_consistency(
                validated_payload,
                fix_arrows=True,
                fix_chains=True,
                fix_directions=True,
        ...
        # --- STAGE 3: Deduplicate functions AFTER evidence validation ---
        if skip_deduplicator:
            # Skip deduplication as requested by user (no progress update needed)
            deduplicated_payload = validated_payload
            ...
            deduplicated_payload = deduplicate_payload(
                validated_payload,
                api_key,
                verbose=False
            )
        ...
        if not skip_fact_checking and FACT_CHECKER_AVAILABLE and api_key:
            ...
            fact_checked_payload = deduplicated_payload

        # --- STAGE 4.5: Post-Validation Finalization (Arrow Notation) ---
        if not skip_schema_validation and SCHEMA_VALIDATOR_AVAILABLE and finalize_interaction_metadata is not None:
            current_step += 1
            update_status(
            ...
            fact_checked_payload = finalize_interaction_metadata(
        ...
        # --- STAGE 4B: Second deduplication pass (catch fact-checker-created dupes) ---
        ...
        # --- STAGE 5: Update PMIDs (runs last to finalize citations) ---
        if not skip_pmid_validation and PMID_UPDATER_AVAILABLE and update_payload_pmids is not None:
            current_step += 1
            update_status(
        ...
        # --- STAGE 5.5: Clean function names (remove "regulation" and generic terms) ---
        if not skip_function_cleaning and FUNCTION_NAME_CLEANER_AVAILABLE and clean_payload_function_names is not None:
            current_step += 1
            update_status(
        ...
        # --- STAGE 6: Save to protein-interaction database ---
        current_step += 1
        update_status(
        ...
        # --- STAGE 6.5: Save to OLD cache format for backward compatibility ---
        current_step += 1
        update_status(
        ...
        # --- STAGE 6.75: Sync to NEW PostgreSQL database ---
        current_step += 1
        update_status(
